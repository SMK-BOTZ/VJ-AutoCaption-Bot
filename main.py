...
try: app_id = int(os.environ.get("app_id", "28243586"))
except Exception as app_id: print(f"⚠️ App ID Invalid {app_id}")
try: api_hash = os.environ.get("api_hash", "4022d5686b9b7a7cf8891205921a0ab3")
except Exception as api_id: print(f"⚠️ Api Hash Invalid {api_hash}")
try: bot_token = os.environ.get("bot_token", "7067057446:AAH_2EEsqjVDkiYYDHWIh1moACkPIYvD-dY")
except Exception as bot_token: print(f"⚠️ Bot Token Invalid {bot_token}")
try: custom_caption = os.environ.get("custom_caption", "`{file_name}`Powered by @SMK_NETWORK")
